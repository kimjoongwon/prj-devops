...
STORYBOOK_BASE_URL="${ADMIN_BASE_URL}/story"
...
CURRENT_OIDC_JWKS_KEYS="${OIDC_JWKS_KEYS:-$(secret_value "${IDP_API_PATH}" "OIDC_JWKS_KEYS")}"
if [[ -z "${CURRENT_OIDC_JWKS_KEYS}" ]]; then
  echo "⚠️ OIDC_JWKS_KEYS is missing in ${IDP_API_PATH}. Export OIDC_JWKS_KEYS before apply to backfill provider signing keys." >&2
fi

CORE_API_SECRET_ARGS=(
  FRONTEND_DOMAIN "${ADMIN_BASE_URL}"
  BACKEND_DOMAIN "${ADMIN_BASE_URL}"
...
upsert_secret "${CORE_API_PATH}" "${CORE_API_SECRET_ARGS[@]}"
...
if [[ -n "${CURRENT_OIDC_JWKS_KEYS}" ]]; then
  IDP_API_SECRET_ARGS+=(
    OIDC_JWKS_KEYS "${CURRENT_OIDC_JWKS_KEYS}"
  )
fi
upsert_secret "${IDP_API_PATH}" "${IDP_API_SECRET_ARGS[@]}"
...
upsert_secret "${IDP_WEB_PATH}" "${IDP_WEB_SECRET_ARGS[@]}"
...
remove_secret_keys "${IDP_WEB_PATH}" \
  OIDC_CLIENT_ID \
  OIDC_CLIENT_SECRET \
  OIDC_REDIRECT_URI
