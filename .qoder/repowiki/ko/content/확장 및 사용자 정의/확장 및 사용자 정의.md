...
이 설정은 ArgoCD가 `helm/applications/plate-api/values-stg.yaml` 파일의 내용을 `values.yaml`의 기본값 위에 오버라이드하도록 지시합니다. 이 방식을 통해 스테이징과 프로덕션 환경 간에 서로 다른 구성(예: 리소스 요청, 이미지 태그, replica 수)을 쉽게 관리할 수 있습니다.
...
1. **Helm 차트에 QA values 파일 추가**:
   - `helm/applications/plate-api/values-qa.yaml` 생성
