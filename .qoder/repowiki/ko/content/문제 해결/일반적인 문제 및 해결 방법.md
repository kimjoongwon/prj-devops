...
2. 관련 Helm 차트의 `values.yaml` 파일 문법 검증:
   ```bash
   helm lint helm/applications/plate-api
   ```
...
- 차트 렌더링 확인:
  ```bash
  helm template helm/applications/plate-api -f helm/applications/plate-api/values-prod.yaml
  ```
- 설치 전 검증:
  ```bash
  helm install --dry-run --debug my-release helm/applications/plate-api
  ```
