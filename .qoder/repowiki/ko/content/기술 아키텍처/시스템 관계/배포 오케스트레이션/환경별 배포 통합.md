...
### 스테이징 환경 구성
스테이징 환경은 `plate-web-stg.yaml` 및 `plate-api-stg.yaml` 파일을 통해 구성됩니다. 이 파일들은 각각 `helm/applications/plate-web` 및 `helm/applications/plate-api` 경로의 Helm 차트를 참조하며, `values-stg.yaml` 값을 사용합니다.
...
### 프로덕션 환경 구성
프로덕션 환경은 `plate-web-prod.yaml` 및 `plate-api-prod.yaml` 파일을 통해 구성됩니다. 이 파일들은 각각 `helm/applications/plate-web` 및 `helm/applications/plate-api` 경로의 Helm 차트를 참조하며, `values-prod.yaml` 값을 사용합니다.
