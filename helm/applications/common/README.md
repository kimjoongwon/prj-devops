...
**통합 관리**: 모든 ingress는 최상위 `helm/templates/ingress.yaml`에서 통합 관리됩니다.

### 파일 구조
```
helm/
├── templates/ingress.yaml          # 통합 ingress 템플릿
├── values.yaml                     # 통합 설정
├── Chart.yaml                      # 메인 차트
└── applications/
...
ingress 설정을 변경하려면 최상위 `helm/values.yaml` 파일을 수정하세요.

### 장점

- **중앙 집중 관리**: 모든 라우팅 규칙을 한 곳에서 관리
